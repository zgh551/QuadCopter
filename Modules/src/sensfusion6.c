...
//add by shiqingziyang 2014.04.03
#ifndef M_PI 
#define M_PI 3.14159
#endif
...
//转化为弧度单位
  ...
  if(!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f)))
  {
    // Normalise accelerometer measurement 单位化
    recipNorm = invSqrt(ax * ax + ay * ay + az * az);//求平方根
    ax *= recipNorm;
    ay *= recipNorm;
    az *= recipNorm;
  ...
  // Integrate rate of change of quaternion四元素更新
  ...
  // Normalise quaternion 规范化四元素
  recipNorm = invSqrt(q0 * q0 + q1 * q1 + q2 * q2 + q3 * q3);//平方根倒数
  q0 *= recipNorm;
  q1 *= recipNorm;
  q2 *= recipNorm;
  q3 *= recipNorm;
