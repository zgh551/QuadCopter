...
*          imu.c - inertial measurement unit    // 惯性测量单元
******************************************************************************
* @attention
* Quadcopter = qdcpt
* COPYRIGHT 2015 
...
void imu6Init(void)
{
 ...
 isMs5611Present = FALSE;
  ...
  mpu6050Reset();//解除休眠，进入工作模式
  vTaskDelay(M2T(200));
  // Activate MPU6050
  mpu6050SetSleepEnabled(FALSE);
  // Enable temp sensor
  mpu6050SetTempSensorEnabled(TRUE);//使能温度传感器
  // Disable interrupts
  mpu6050SetIntEnabled(FALSE);//关闭中断
...
void imu6Read(Axis3f* gyroOut, Axis3f* accOut)
{
  ...
  imuAddBiasValue(&gyroBias, &gyroMpu); // 陀螺仪的偏差一直更新，将陀螺仪的数据放入缓冲
      ...
      !accelBias.isBiasValueFound)
  {
    ...
    accelBias.bias.z = mean.z - IMU_1G_RAW; // 减去重力加速度
...
/**
 * Calculates the variance and mean for the bias buffer.计算方差和平均值
 */
static void imuCalculateVarianceAndMean(BiasObj* bias, Axis3i32* varOut, Axis3i32* meanOut)
{
  uint32_t i;
...
/**
 * Adds a new value to the variance buffer and if it is full    将新值加入偏差缓冲，如果缓冲满则替换最老的值，
 * replaces the oldest one. Thus a circular buffer.                     所以这是一个循环缓冲
 */
static void imuAddBiasValue(BiasObj* bias, Axis3i16* dVal)
{
...
/**
 * Checks if the variances is below the predefined thresholds.  检查偏差是否在预定值以下
 * The bias value should have been added before calling this.       在调用前应该加上偏差值
